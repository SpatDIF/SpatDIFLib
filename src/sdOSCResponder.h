...
inline std::vector<sdOSCMessage> sdOSCResponder::getAction(std::string command, sdOSCMessage message){
    ...
    if(command == "getEventSetsFromAllEntities"){
        ...
        std::vector<std::pair<const sdEntity* , std::shared_ptr<sdProtoEvent>>>::iterator rit = reports.begin();
        while(rit != reports.end()){
            ...
            switch (event->getDescriptor()){
                case SD_POSITION:
                case SD_ORIENTATION:{
                    auto value = event->getValueAsString();
                    returnMessage.appendFloats(stringToFloats(value));
